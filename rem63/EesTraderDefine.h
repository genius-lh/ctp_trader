...
#define SL_EES_API_VERSION    "3.1.3.63"				///<  api版本号
...
#define EES_OrderState_order_accept             0       ///< =order accept（普通接受）
#define EES_OrderState_order_triggered          1		///< =order triggered（止盈止损单触发）
#define EES_OrderState_order_suspended          2		///< =单子挂起
#define EES_OrderState_order_active             3		///< =单子激活
...
{ 
	...
	EES_OrderState      m_OrderState;					///< 订单状态 0=order accept（普通接受）；1=order triggered（止盈止损单触发）；2=单子挂起; 3=单子激活;
...
struct EES_CancelOrder
{
	...
	EES_ClientToken  m_ClientOrderToken;				///< 原来本地单号
	unsigned int    m_origOrderUserId;                  ///可以为指定的user_Id 也可以为0 0会把登录的user_id自动补充
	EES_CancelOrder()
	{
		memset(this, 0, sizeof(*this));
	}
