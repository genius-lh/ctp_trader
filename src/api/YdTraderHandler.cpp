...
void CYdTraderHandler::notifyOrder(const YDOrder *pOrder,const YDInstrument *pInstrument,const YDAccount *pAccount)
{
  ...
  // 本地报单编号
  snprintf(traderOrder.UserOrderLocalID, sizeof(traderOrder.UserOrderLocalID), "%d", pOrder->OrderRef);
  // 买卖
...
void CYdTraderHandler::notifyTrade(const YDTrade *pTrade,const YDInstrument *pInstrument,const YDAccount *pAccount)
{
  ...
  ///本地报单编号
  snprintf(traderTrade.UserOrderLocalID, sizeof(traderTrade.UserOrderLocalID), "%d", pTrade->OrderRef);
  ///交易日
...
void CYdTraderHandler::QryInstrument()
{
  ...
  for(int i = 0; i < m_TraderApi->getInstrumentCount(); i++) {
    ...
    trader_trader_api_on_rsp_qry_instrument(self, &traderInstrument, 0, NULL, !(m_TraderApi->getInstrumentCount()-1-i));
  }
...
{
  trader_trader_api* self = (trader_trader_api*)m_Arg;
  const YDPrePosition *pPrePosition;
  ...
  for(int i = 0; i < m_TraderApi->getPrePositionCount(); i++) {
    pPrePosition = m_TraderApi->getPrePosition(i);
    if(!pPrePosition){
      continue;
    }

    CMN_INFO(
      "pPrePosition->m_pInstrument->InstrumentID=[%s]\n"
      "pPrePosition->PositionDirection=[%d]\n"
      "pPrePosition->PrePosition=[%d]\n"
      , pPrePosition->m_pInstrument->InstrumentID
      , pPrePosition->PositionDirection
      , pPrePosition->PrePosition
    );

    filter.PositionDate = 0;
    filter.PositionDirection = pPrePosition->PositionDirection;
    filter.HedgeFlag = pPrePosition->HedgeFlag;
    filter.pInstrument = pPrePosition->m_pInstrument;
    ...
    for(int j = 0; j < count;i++){
      ...
      strncpy(traderPosition.InstrumentID, pPrePosition->m_pInstrument->InstrumentID, sizeof(traderPosition.InstrumentID));
      traderPosition.PositionDate = '0' + positions[j]->PositionDate;
      traderPosition.PosiDirection = '0' + positions[j]->PositionDirection;

      if(pPrePosition->m_pInstrument->m_pExchange->UseTodayPosition){
      ...
      trader_trader_api_on_rsp_qry_investor_position(self, &traderPosition, 0, NULL, (i == (m_TraderApi->getPrePositionCount()-1)));
    }
